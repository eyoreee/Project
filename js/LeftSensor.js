class LeftSensor extends Sensor {
  ...
  setSensorPos(tempX, tempY) {
    ...
    const sensorCol = Math.floor(this.senX / 10);
    const startRow = Math.floor(this.senY / 10);
    const endRow = Math.floor((this.senY + this.height - 1) / 10);

    // 센서가 속한 칸을 기준으로 3*3 범위 체크
    for (let i = startRow; i <= endRow; i++) {
      for (let j = sensorCol - 1; j <= sensorCol + 1; j++) {
        // 블럭배열이 false(0)이 아니고 충돌이 일어났을 때 수행
        if (
          blockArray[i][j] &&
          collisionCheckBySensor(this, blockArray[i][j])
        ) {
          this.taxi.x = blockArray[i][j].x + blockArray[i][j].width;
          this.taxi.velX = 0;
          return true; // 충돌 했음
        }
      }
